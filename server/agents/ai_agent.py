...
class AIAgent:
    ...
    def __init__(self, system_message: str, tool_names: list[str] = []):
        ...
        self.llm = get_deepseek_llm()            # streaming=False inside
        ...
        # NOTE: self.agent_executor is not assigned here by default.
        # Subclasses typically build an agent executor themselves.

    def run(self, message: str) -> str:
        """
        Executes the AI agent for inference and gets a response as a result.
        (Most derived classes override this anyway.)
        """
        if not hasattr(self, "agent_executor"):
            raise ValueError("No agent_executor defined in AIAgent.")
    ...
    def _create_agent_tools(self, tool_names=[]) -> list[Tool]:
        """
        Returns a list of agent tools from the toolbox, filtered by 'tool_names'.
        """
        db_name = MongoDBClient.get_db_name()
        ...
        return community_tools + custom_tools
